...
import subprocess
import asyncio
import httpx # 🚀 THÊM THƯ VIỆN NÀY ĐỂ GỌI API MẠNG
...
# ============================================================
# 🚀 HÀM MỚI: TÌM KIẾM LỜI BÀI HÁT SIÊU TỐC QUA INTERNET
# ============================================================
def fetch_lyrics_online(query: str, output_path: str) -> bool:
    """Quét dữ liệu từ LRCLIB (Thư viện Lyrics toàn cầu)"""
    try:
        # Làm sạch tên file để làm từ khóa tìm kiếm (Bỏ dấu gạch dưới)
        search_query = query.replace('_', ' ').replace('-', ' ').strip()
        url = "https://lrclib.net/api/search"
        
        # Dùng httpx Client đồng bộ để gọi API
        with httpx.Client(timeout=10.0) as client:
            res = client.get(url, params={"q": search_query})
            if res.status_code == 200:
                data = res.json()
                if data and len(data) > 0:
                    # Ưu tiên lấy kết quả đầu tiên có chứa plainLyrics (Lời văn bản)
                    for track in data:
                        if track.get("plainLyrics"):
                            with open(output_path, "w", encoding="utf-8") as f:
                                f.write(f"🎤 BÀI HÁT: {track.get('trackName', 'Unknown')} - {track.get('artistName', 'Unknown')}\n")
                                f.write("="*50 + "\n\n")
...
def process_audio_pipeline(file_path: str, clean_name: str, task_id: str, ext: str, separate_beat: bool, extract_lyrics: bool):
    ...
    lyrics_output = os.path.join(project_dir, f"{task_id}_lyrics.txt")
    
    # ------------------------------------------------------------
    # 🛠️ GIAI ĐOẠN 0: TRÍCH XUẤT ÂM THANH TỪ VIDEO
    # ------------------------------------------------------------
    ...
    if separate_beat:
        ...
        try:
            ...
            err_msg = e.stderr.decode('utf-8') if e.stderr else str(e)
            print(f"❌ Lỗi nội bộ tại Demucs: {err_msg}")
        finally:
            if os.path.exists(temp_demucs_dir):
                shutil.rmtree(temp_demucs_dir)

    # ------------------------------------------------------------
    # 🛠️ GIAI ĐOẠN 2: TÌM LỜI BÀI HÁT (INTERNET + WHISPER DỰ PHÒNG)
    # ------------------------------------------------------------
    if extract_lyrics:
        print(f"📝 [Audio Engine] Đang tìm kiếm lời bài hát cho: {clean_name}")
        
        # 🚀 CHIẾN THUẬT 1: Quét dữ liệu mạng siêu tốc (Tốn 0.5 giây)
        is_lyric_found = fetch_lyrics_online(clean_name, lyrics_output)
        
        if is_lyric_found:
            print(f"⚡ Đã tải lời bài hát từ Internet thành công! Bỏ qua Whisper để tiết kiệm thời gian.")
        else:
            # 🚀 CHIẾN THUẬT 2: Nếu mạng không có, khởi động Whisper AI (Chậm nhưng chắc chắn)
            print("⚠️ Không tìm thấy lời bài hát trên mạng. Khởi động Whisper AI để tự động nghe và chép lời...")
            ...
            try:
                ...
                if os.path.exists(generated_txt):
                    ...
                    print("✅ Whisper AI đã chép lời xong!")
            except subprocess.CalledProcessError as e:
                err_msg = e.stderr.decode('utf-8') if e.stderr else str(e)
                print(f"❌ Lỗi nội bộ tại Whisper STT: {err_msg}")
            finally:
                if os.path.exists(temp_whisper_dir):
                    shutil.rmtree(temp_whisper_dir)

    # ============================================================
    # 🏁 KẾT THÚC PIPELINE: Đóng cờ báo hiệu (Kiểm tra nghiêm ngặt)
    # ============================================================
    try:
        # 🚀 CHỈ BÁO XONG KHI FILE MP3 THỰC SỰ ĐÃ ĐƯỢC AI TẠO RA THÀNH CÔNG
        is_success = False
        ...
        if is_success:
            ...
            flag_file = os.path.join(project_dir, f"{task_id}_completed.txt")
            with open(flag_file, "w", encoding="utf-8") as f:
                f.write("DONE")
        else:
            print(f"❌ [Audio Engine] Tiến trình AI thất bại giữa chừng. Hủy tạo cờ giao diện cho: {task_id}")
...
async def extract_audio_features(
    ...
    try:
        ...
        task_id = f"{clean_name}_{timestamp}"
        ...
        return JSONResponse(status_code=202, content={
            "status": "processing",
            "message": f"Hệ thống đã tiếp nhận dữ liệu định danh: '{task_id}'",
            ...
            "expected_outputs": {
                ...
                "lyrics": f"/audio-files/{clean_name}/{task_id}_lyrics.txt" if extract_lyrics else None
            }
        })
...
async def check_audio_status(song_name: str, task_id: str):
    flag_path = os.path.join(OUTPUT_DIR, song_name, f"{task_id}_completed.txt")
