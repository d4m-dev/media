  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mx-2">
    <?php foreach ($songs as $song): ?>
      <div class="card p-4 border rounded-lg shadow-lg flex items-center space-x-4 relative">
        <a href="player.php?id=<?= $song['id'] ?>" class="flex items-center w-full" onclick="event.preventDefault(); loadSong(<?= htmlspecialchars(json_encode($song)) ?>);">
          <div class="flex items-center w-full">
            ...
            <div class="flex-1 ml-2">
              ...
              <p class="text-gray-400">Lượt nghe: <?= $song['views'] ?? 0 ?></p>
            </div>
          </div>
        </a>
