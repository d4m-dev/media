...
'<br /><small>' . $lng['minmax_2_100'] . '</small></p>' .
'<p><h3>' . $lng['description'] . '</h3>' .
'<textarea name="desc" rows="' . $set_user['field_h'] . '"></textarea>' .
'<br /><small>' . $lng['not_mandatory_field'] . '<br />' . $lng['minmax_2_500'] . '</small></p>' .'<div class="phdr">Đặt chuyên mục vip:</div><input type="radio" value="1" name="vip"/>ON<br/><input type="radio" value="0" name="vip"/>OFF' .
...
$sort = $res_s['realid']+1;
// Меняем категорию
mysql_query("UPDATE `forum` SET `refid` = '$category', `realid` = '$sort',`vip`='$vip' WHERE `id` = '$id'");
...
echo '</select></p><div class="phdr">Đặt chuyên mục vip:</div><input type="radio" value="1" name="vip"/>ON<br/><input type="radio" value="0" name="vip"/>OFF';
}
