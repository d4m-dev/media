...
$history_query = "SELECT songs.id, songs.title, songs.artist, songs.cover_path, listening_history.listened_at
...
<div class="container mx-auto p-4">
  <h2 class="text-2xl font-bold text-green-500 mb-6">Lịch Sử Nghe Nhạc</h2>
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
    <?php foreach ($history as $song): ?>
      <div class="card p-4 border rounded-lg bg-white shadow-md">
        ...
        <p class="text-sm text-gray-400">Đã nghe vào: <?= date('d/m/Y H:i', strtotime($song['listened_at'])) ?></p>
        <a href="../pages/player.php?id=<?= $song['id'] ?>" class="text-green-500 hover:underline">Nghe lại</a>
      </div>
    <?php endforeach; ?>
  </div>
</div>
